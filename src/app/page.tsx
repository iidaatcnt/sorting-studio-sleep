...
export default function SleepSortStudio() {
  ...
  const reset = useCallback(() => {
    const newElements = Array.from({ length: ARRAY_SIZE }, (_, i) => ({
      id: i,
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-indigo-500 rounded-lg flex items-center justify-center shadow-lg shadow-indigo-500/20">
              <Moon className="text-slate-950 w-5 h-5 fill-current" />
            </div>
            <h1 className="font-black italic tracking-tighter text-xl uppercase tracking-widest text-indigo-400">Sleep_Sort_Studio</h1>
          </div>
          <div className="flex items-center gap-6">
            <a href="https://github.com/iidaatcnt/sorting-studio-sleep" target="_blank" rel="noreferrer" className="text-slate-600 hover:text-white transition-colors">
              <Github size={20} />
            </a>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
        <div className="lg:col-span-8 flex flex-col gap-8">
          ...
          <div className="relative aspect-video lg:aspect-square max-h-[550px] bg-slate-900/40 rounded-[3rem] border border-white/5 p-16 flex flex-col gap-8 overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 flex flex-col gap-4">
              <div className="text-[10px] mono text-slate-700 font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                <Moon size={12} /> Zzz... Sleeping Area
              </div>
              <div className="grid grid-cols-1 gap-3">
                {elements.map((el) => (
                  <div key={el.id} className="relative h-12 bg-white/5 rounded-2xl border border-white/5 flex items-center px-6 gap-6 overflow-hidden">
                    <div className="w-8 mono text-xs font-black text-slate-500">{el.value}</div>
                    ...
                    <div className="w-20 text-right">
                      {el.status === 'sleeping' ? (
                        <span className="text-[10px] mono text-slate-600 animate-pulse">SLEEPING</span>
                      ) : (
                        <span className="text-[10px] mono text-amber-400 font-bold">WOKE UP!</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            {/* Sorted Result Zone */}
            <div className="h-24 border-t border-white/5 pt-8 flex items-center gap-3">
              <div className="text-[10px] mono text-slate-700 font-black uppercase tracking-widest mr-4">Sorted:</div>
                ...
                <AnimatePresence>
                  {sortedList.map((val, idx) => (
                    <motion.div
                      ...
                      className="w-10 h-10 bg-indigo-500/20 border border-indigo-500/30 rounded-xl flex items-center justify-center mono text-xs font-black text-indigo-400"
                    >
                      {val}
                    </motion.div>
                  ))}
            ...
            <div className="flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <div className="flex items-center justify-between mono text-[10px] text-slate-500 uppercase font-black tracking-widest mb-3">
                  ...
                  <span className="text-indigo-400">x{speed.toFixed(1)}</span>
                </div>
                <div className="flex gap-4 items-center">
                  <input type="range" min="0.5" max="5" step="0.1" value={speed} onChange={(e) => setSpeed(parseFloat(e.target.value))} className="flex-1 appearance-none bg-slate-800 h-1.5 rounded-full accent-indigo-500 cursor-pointer" />
                </div>
              </div>
            </div>

            <div className="p-6 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 flex gap-4 text-slate-400">
              <div className="mt-1 p-2 bg-indigo-500/10 rounded-xl shrink-0">
                <AlarmClock size={16} className="text-indigo-400" />
              </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          <div className="p-10 bg-zinc-900/80 border border-white/5 rounded-[3rem] shadow-2xl">
            ...
            <div className="p-6 bg-black/40 rounded-3xl border border-white/5 mb-8">
              <h3 className="text-indigo-400 font-black mb-3 text-sm">Sleep Sort</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mono text-[9px] font-black uppercase tracking-tighter">
              <div className="p-4 bg-white/5 rounded-2xl text-center border border-white/5 hover:border-indigo-500/20 transition-colors">
                ...
                <span className="text-indigo-300">O(max(N)) pt</span>
              </div>
              <div className="p-4 bg-white/5 rounded-2xl text-center border border-white/5 hover:border-indigo-500/20 transition-colors">
                <span className="text-slate-600 block mb-1">Status</span>
                <span className="text-rose-400">Joke / Legend</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-10 bg-black border border-white/5 rounded-[3rem] flex-1 flex flex-col min-h-[450px]">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-black text-[10px] uppercase tracking-widest text-slate-500">Source_Kernel</h2>
              </div>
      ...
      <footer className="mt-20 border-t border-white/5 py-16 text-center relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-6">
          <Sun className="text-slate-900 w-8 h-8 opacity-20" />
